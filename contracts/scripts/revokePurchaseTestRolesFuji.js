...
async function main() {
  ...
  for (const [label, role] of [
    ...
    const currentlyGranted = await fund.hasRole(role, DEPLOYER_EOA);
    if (!currentlyGranted) {
      console.log(`Skipping ${label}, deployer does not hold it`);
      continue;
    }

    console.log(`Revoking ${label} from ${DEPLOYER_EOA}...`);
    const tx = await fund.revokeRole(role, DEPLOYER_EOA);
    await tx.wait();
  }
