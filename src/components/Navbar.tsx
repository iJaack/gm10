...
export default function Navbar() {
    ...
    return (
        <>
            ...
            >
                <div className={`max-w-[1400px] mx-auto flex items-center justify-between transition-all duration-300 ${scrolled ? 'py-4 px-6 md:px-8' : 'py-6 px-6 md:px-8'}`}>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {/* Mobile Menu Toggle */}
                        <button
                            className="md:hidden relative z-[110] w-10 h-10 flex items-center justify-center text-white bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all"
            ...
            {/* Mobile Menu Overlay */}
            {mobileMenuOpen && (
                <div className="md:hidden fixed inset-0 bg-[#0a0f1c] z-[100] flex flex-col p-6 pt-32 animate-in fade-in duration-200">
                    <ul className="flex flex-col gap-2 list-none m-0 p-0 overflow-y-auto">
                        {navLinks.map((link) => (
                            <li key={link.path}>
                                <Link
                                    to={link.path}
                                    className={`block px-6 py-4 rounded-2xl text-2xl font-bold transition-all ${location.pathname === link.path
                    ...
                    <div className="mt-auto pt-10 pb-12 flex flex-col items-center gap-6 border-t border-white/10">
                        <ConnectButton
                            accountStatus="address"
                            chainStatus="icon"
                            showBalance={true}
                        />
                        <p className="text-xs text-white/30 font-medium tracking-widest uppercase">Gem Mint Strategy</p>
                    </div>
                </div>
            )}
        </>
