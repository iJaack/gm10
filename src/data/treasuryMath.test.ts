...
describe('treasury math', () => {
    ...
    it('uses stable accounting before native wallet balances', () => {
        expect(resolveLiquidTreasuryUsdt6({
            walletBalancesWei: [1n * AVAX, 1n * AVAX],
            avaxUsd: 9.29,
            stableAccountingLiquidTreasury: 8_856_316_567n,
        })).toBe(8_856_316_567n);
    });

    it('falls back to native wallet balances when stable accounting is unavailable', () => {
        expect(resolveLiquidTreasuryUsdt6({
            walletBalancesWei: [1n * AVAX, 1n * AVAX],
            avaxUsd: 9.29,
        })).toBe(18_580_000n);
